...
%---------------------------------------------------------------------------------
    switch type
        ...
        case 'w2*gauss'
         ...
         case 'w3*gauss'
            gt = @(t) s^(-1/2)*pi^(-1/4).*(t).*exp(-t.^2/s^2/2);        %wrong!unused
            gf = @(w) sqrt(2 * s)*pi^(1/4)*exp(-(s * w).^2/2).*w.^3;
         case 'w4*gauss'
            gt = @(t) s^(-1/2)*pi^(-1/4).*(t).*exp(-t.^2/s^2/2);        %wrong!unused
            gf = @(w) sqrt(2 * s)*pi^(1/4)*exp(-(s * w).^2/2).*w.^4;
        case 'w1*1ord(w)_gauss'
            gt = @(t) s^(-5/2)*pi^(-1/4).*(-t.^2).*exp(-t.^2/s^2/2);    %wrong!unused
            gf = @(w) -sqrt(2)*s^(5/2)*pi^(1/4)*exp(-(s * w).^2/2).*w.^2;
        case 'w2*1ord(w)_gauss'
            gt = @(t) s^(-5/2)*pi^(-1/4).*(-t.^2).*exp(-t.^2/s^2/2);    %wrong!unused
            gf = @(w) -sqrt(2)*s^(5/2)*pi^(1/4)*exp(-(s * w).^2/2).*w.^3;  
